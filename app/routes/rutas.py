...
Conductores + RutaDespacho + manifiesto por ruta.
"""
...
def crear_conductor():
    ...
    c = Conductor(
        ...
        placa=data.get('placa', '').strip().upper() or None,
    )
    db.session.add(c)
    db.session.commit()
...
def actualizar_conductor(id):
    ...
    if 'placa'    in data: c.placa    = data['placa'].strip().upper() or None
...
@jwt_required()
def listar_rutas():
    """Historial de rutas — filtros opcionales: fecha, conductor_id, estado."""
...
def crear_ruta():
    ...
    data = request.get_json()
    for campo in ['conductor_id', 'tipo_ruta']:
        if not data.get(campo):
            return jsonify({'error': f'Campo requerido: {campo}'}), 400
