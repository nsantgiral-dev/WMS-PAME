...
class ConnektaConsultasGateway:
    ...
    #: `ConnektaPaginacionError` en vez de devolver lo leído. El tope existe
    #: para que un filtro roto no barra la tabla entera de movimientos, no para
    ...
    def get_rowids_factura(self, tipo_docto_fe: str, consec_fe) -> list:
        ...
        try:
            ...
            # ── Barrido paginado (re-aplicado en el merge del 2026-09-11) ──
            #
            # Esta función pedía UNA página de 100 y devolvía eso como si fuera
            # la factura entera. El refactor que extrajo este gateway movió la
            # versión anterior al arreglo, así que el defecto volvió con él.
            #
            # Lo que se pierde con una factura de más de 100 líneas: la base
            # gravable sale corta —el DC declara menos retención de la que el
            # cliente retuvo, diferencia contra la DIAN— y la NC total cruza
            # menos que el saldo, así que la factura queda abierta en cartera.
            # No avisa: una página llena es indistinguible de una factura corta.
            rows = []
            _paginas_leidas = 0
            for pag in range(1, self._MAX_PAGINAS_FACTURA + 1):
                ...
                # Va por PÁGINA: un rechazo en la página 2 leído como «se
                # acabaron las líneas» devolvería las 100 primeras en silencio.
    ...
    def get_cxc_general(self, nit: str) -> list:
        ...
        try:
            # ── Barrido paginado (re-aplicado en el merge del 2026-09-11) ──
            #
            # Pedía UNA página de 100 filas y la devolvía como si fuera la
            # cartera completa. Con un cliente de más de 100 filas abiertas
            # —el mayorista de ruta lo es—, `cxc_cruce.esta_saldada` responde
            # `None`, el pre-flight no bloquea, y sale un SEGUNDO recibo de
            # caja sobre una factura ya saldada.
            filas = []
            ...
            # política de ESTA función es la contraria a la de
            # `get_rowids_factura`, y a propósito: no debe bloquear el recibo
            # de caja. `[]` significa «no sé», y el caller ya sabe tratarlo;
            # 100 filas parciales **afirman** una cartera que no es.
