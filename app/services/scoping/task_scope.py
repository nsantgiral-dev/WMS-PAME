...
def scope_packing(usuario, query):
    ...
    # Empacador / operario: filtra por almacen_id.
    # Tanto las TareasPackings de PEDIDO como las de TRASLADO tienen almacen_id
    # igual al almacén origen (el que empaca). Un mismo filtro cubre ambos tipos —
    # no hay necesidad de separar por tipo_documento.
    if usuario.almacen_id:
        return query.filter_by(almacen_id=usuario.almacen_id)
    return query
