"""
Ciclo COMPLETO real contra Siesa QA para uno de los 7 casos elegidos
(conversacion 2026-09-04): Pedido -> Picking -> Packing (244328/142945/142943)
-> Ruta (atajo, el muelle ya se probo de punta a punta con PD1113) ->
Conductor (confirmar_parada, segun el escenario de CASOS) -> Liquidacion
(RC/NC/DC via liquidar_ruta_siesa + _ejecutar_job por cada job encolado).

Base LOCAL AISLADA por pedido (nunca la Postgres de produccion), Siesa QA
real (.env.qa). `disparar_dlq_inmediato` desactivado -- unico disparador es
este script.

Uso:
    venv/Scripts/python.exe scripts/qa_ciclo_completo.py PD1125
    venv/Scripts/python.exe scripts/qa_ciclo_completo.py PD1125 --disparar-real --si-de-verdad
"""
import argparse
import os
import sys
...
CASOS = {
    'PD1125': {'tipo_docto': 'PD', 'consec_docto': 1125, 'cliente': 'BENITEZ LOPEZ KAREN DAYANA',
               ...
               'entrega_kind': 'PARCIAL', 'entregado_frac': 0.9},
    ...
    'PD1146': {'tipo_docto': 'PD', 'consec_docto': 1146, 'cliente': 'CASTRO CUBILLOS PAULA ANDREA',
               ...
               'entrega_kind': 'TRANSFERENCIA'},
}


def seed_y_despachar(db, connekta, caso, numero_pedido, admin_email):
    ...
    tarea = PackingService.crear_desde_picking(
        ...
        almacen_id=almacen.id, tipo_docto_pedido_siesa=caso['tipo_docto'],
    ...
    PackingService.cerrar_packing(tarea.id, [{'tipo': 'Caja', 'cantidad': 1}], operario.id)
    db.session.commit()

    # ── Despacho real (244328 -> 142945 -> 142943) ──
    ...
    if not tarea.siesa_triggered:
        # Recuperación (mismo patrón que PD1113): la RM puede haberse creado
        # igual aunque el job no haya terminado (timeout, reintento, etc.) —
        # reconciliar contra Siesa real antes de reintentar 142943.
        rm = connekta.get_remision_desde_pedido(caso['tipo_docto'], caso['consec_docto'])
    ...
    # ── Ruta (atajo — el muelle real ya se probó con PD1113) ──
    bultos = Bulto.query.filter_by(tarea_id=tarea.id).all()
    ...
    return tarea, ruta, productos, operario.id, conductor.id, admin.id
...
def main():
    ...
    os.environ['DATABASE_URL'] = 'sqlite:///' + os.path.join(REPO_ROOT, 'scripts', f'qa_{args.pedido.lower()}.db')
    ...
    with app.app_context():
        ...
        from app.services.liquidacion_service import (
            LiquidacionService, CATALOGO_RETENCIONES, monto_de_retencion)
        from app.services.siesa_job_service import _ejecutar_job
        from app.models.siesa_job import SiesaJob
        ...
        print('--- Despacho ---')
        tarea, ruta, productos, operario_id, conductor_id, admin_id = seed_y_despachar(
            db, connekta, caso, numero_pedido, f'admin_qa_{args.pedido.lower()}@wms-pame.local')

        if not tarea.fe_tipo or not tarea.fe_consec:
            # fe_resolver ya lo intenta dentro de confirmar_parada, pero acá
            # se imprime aparte para tener el monto real ANTES de declarar la entrega.
            from app.services.fe_resolver import resolver_fe
            try:
                fe_tipo, fe_consec = resolver_fe(tarea)
            except Exception as e:
                print('[ERROR] no se pudo resolver la FE:', e); sys.exit(1)
        else:
            fe_tipo, fe_consec = tarea.fe_tipo, tarea.fe_consec

        bruto, iva, neto, lineas = obtener_factura_real(connekta, fe_tipo, fe_consec)
        print(f'  FE {fe_tipo}-{fe_consec}: bruto={bruto} iva={iva} neto={neto}')

        # ── Confirmar parada según el escenario ──
        kind = caso['entrega_kind']
        datos = {}
        if kind == 'PARCIAL':
            frac = caso.get('entregado_frac', 0.9)
            total_ped = caso['items'][0]['cantidad']
            entregado = round(total_ped * frac)
            devuelto = total_ped - entregado
            monto_cobrado = round(neto * (entregado / total_ped), 2)
            datos = {
                'estado_entrega': 'PARCIAL', 'forma_pago': 'EFECTIVO',
                'monto_cobrado': monto_cobrado,
                'observaciones': f'Cliente recibio {entregado} de {total_ped}',
                'items_entregados': [{
                    'codigo': productos[0].codigo, 'nombre': productos[0].nombre,
                    'unidad': 'und', 'cantidad_pedida': total_ped,
                    'cantidad_entregada': entregado,
                }],
            }
        elif kind == 'RECHAZADO':
            datos = {'estado_entrega': 'RECHAZADO', 'motivo_rechazo': 'CLIENTE_CERRADO',
            ...
            datos = {'estado_entrega': 'ENTREGADO', 'forma_pago': 'TRANSFERENCIA',
                     'monto_cobrado': neto}

        print('--- Confirmar parada ---', datos)
        recaudo_id, _ = RutaService.confirmar_parada(ruta.id, tarea.id, conductor_id, datos)
        db.session.commit()

        print('--- Liquidar ruta (encola jobs) ---')
        resumen = LiquidacionService.liquidar_ruta_siesa(ruta.id, admin_id=admin_id)
        print('  resumen:', resumen)

        # PARCIAL/RECHAZADO arman una DevolucionCliente ABIERTA (no la NC
        # todavía) — la NC solo se encola cuando recepción confirma que la
        # mercancía volvió físicamente, igual que en la operación real.
        if kind in ('PARCIAL', 'RECHAZADO'):
            ...
            devolucion = DevolucionCliente.query.filter_by(recaudo_entrega_id=recaudo_id).first()
            if devolucion:
                print(f'  confirmando entrada física de devolución {devolucion.id}...')
                DevolucionClienteService.confirmar_entrada_fisica(
                    devolucion.id, recepcionista_id=operario_id)
                db.session.commit()

        # Camino real del DLQ (mismo `procesar_jobs_pendientes` que corre el
        # scheduler cada 5 min) — maneja PROCESANDO/COMPLETADO/FALLIDO y la
        # dependencia NC→RC (`DependenciaPendiente`) de verdad, en vez de
        # llamar `_ejecutar_job` a mano sin marcar estado.
        from app.services.siesa_job_service import procesar_jobs_pendientes
        for vuelta in range(4):
            # Forzar `proximo_intento` a ahora — `DependenciaPendiente` lo
            # reprograma 30 min adelante por defecto; acá ya sabemos que la
            # dependencia (NC) puede haberse resuelto en la vuelta anterior.
