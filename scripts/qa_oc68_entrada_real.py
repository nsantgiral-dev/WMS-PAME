"""
Entrada real (142948) contra OC68 (003-OC-68), creada por el usuario el
2026-09-10 para reponer BELLESB1382 (disponible=0 en NB1, bloqueaba el lote
de 16 pedidos). DISPAPELES SAS, comprador FIGUEROA ANACONA NELLY CARMENSA
(NIT 52430291), 100 unidades pedidas -- se confirma entrada COMPLETA.

Base LOCAL AISLADA (nunca Postgres de produccion), Siesa QA real (.env.qa).

Uso:
    venv/Scripts/python.exe scripts/qa_oc68_entrada_real.py --si-de-verdad
"""
import argparse
import os
import sys
...
def main():
    ...
    os.environ.setdefault('SECRET_KEY', 'qa-oc68-entrada-32-bytes-o-mas-xx')
    ...
    with app.app_context():
        ...
        if not antes:
            print('ERROR: no se encontró la línea real de OC68 — abortando sin tocar nada.')
            sys.exit(2)
        ...
        print('\nRESULTADO: el POST real (142948) respondió codigo:0 contra OC68 real.')
