...
class TestCobraEnLaPuerta:
    ...
    def test_ausente_no_se_afirma(self):
        assert cp.cobra_en_la_puerta('', self.CONTADO, self.RUTA) is None
        assert cp.cobra_en_la_puerta(None, self.CONTADO, self.RUTA) is None

    def test_sin_condicion_de_ruta_configurada_no_revienta(self):
        """`cond_pago_ruta=''` (SIESA_COND_PAGO_RUTA sin configurar) no puede
        hacer que todo compare contra un string vacío y dé falsos positivos."""
        assert cp.cobra_en_la_puerta('30D', self.CONTADO, '') is False
