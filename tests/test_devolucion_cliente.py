...
def _linea_input(producto, cantidad_facturada=10, cantidad_devuelta=4,
                 es_averiado=False, rowid='999'):
    """Un ítem devuelto tal como lo manda la pantalla.

    `rowid` se volvió parámetro en el merge del 2026-09-11. Antes iba fijo en
    `'999'` para todos y no importaba: `_construir_lineas_nc` cruzaba por
    referencia. Ahora cruza por `f470_rowid` —el arreglo del producto de doble
    unidad, que sale en la factura como dos líneas con la MISMA referencia y
    distinta unidad—, así que dos ítems con el mismo rowid son una colisión y
    uno se pierde.

    El default se conserva porque los demás tests devuelven un solo ítem y ahí
    da igual. Donde hay más de uno, hay que pasar el rowid de SU línea: es lo
    que pasa en una factura real.
    """
    return {
        ...
        'f150_id_bodega': 'NB1', 'f470_rowid': rowid,
    }
